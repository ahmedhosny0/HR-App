...
SELECT
    ...
    e.HireDate,
r.MissionPercentage,
Assemp.EmployeeName AssignEmployee,
    rt.Name AS TypeName,
...
JOIN HR_Employees Assemp
ON r.ActingEmployeeId = Assemp.EmployeeId
...
AND a.Status = 1  "; // 1 يعني Pending
                ...
                while (rdr.Read())
                {
                    pendingRequests.Add(new PendingRequestVM
                    {
                        ...
                        AssignEmployee = rdr["AssignEmployee"].ToString(),
                        JobTitle = rdr["JobTitle"].ToString(),
                        RequestTypeId = (int)rdr["RequestTypeId"],
                        MissionPercentage = (decimal)rdr["MissionPercentage"],
